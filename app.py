...
chlorides = st.slider("Chlorides（氯化物）", float(df["chlorides"].min()), float(df["chlorides"].max()), format="%.3f", step=0.001)
...
density = st.slider("Density（密度）", float(df["density"].min()), float(df["density"].max()), format="%.4f", step=0.0001)
